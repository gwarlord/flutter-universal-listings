...
exports.cleanupEnhancements = exports.saveEnhancementVariant = exports.enhancePhoto = void 0;
 ...
 * }
 */
exports.enhancePhoto = functions
    ...
    const { listingId, imageUrl, category, subscriptionTier, enhancements, includeDisclosure, usePreview = false, } = data;
    // Validate inputs
    if (!listingId || !imageUrl || !category || !subscriptionTier) {
        throw new functions.https.HttpsError("invalid-argument", "Missing required fields: listingId, imageUrl, category, subscriptionTier");
    }
    try {
        ...
        if (subscriptionTier !== "free") {
            ...
            if (enhancements.includes("studio_background")) {
                ...
                enhancedBuffer = await applyStudioBackground(enhancedBuffer, visionResults, usePreview);
                appliedEnhancements.push("studio_background");
            }
        }
        // Professional Plus features
        if (subscriptionTier === "professional_plus" ||
        ...
        const enhancedUrl = await uploadEnhancedImage(enhancedBuffer, listingId, `enhanced-${(0, uuid_1.v4)()}.jpg`);
        ...
        throw new functions.https.HttpsError("internal", `Enhancement failed: ${error}`);
    }
});
/**
 ...
 */
async function analyzeImageWithVision(imageBuffer, category) {
    try {
        const request = {
 ...
 */
async function applyAutoCrop(imageBuffer, visionResults) {
    try {
 ...
 */
async function applyLightingNormalization(imageBuffer) {
    try {
 ...
 */
async function applyClarity(imageBuffer) {
    try {
        return await (0, sharp_1.default)(imageBuffer)
            .sharpen({ sigma: 0.5 })
            .withMetadata()
 ...
 */
async function applyBackgroundBlur(imageBuffer, visionResults) {
    try {
 ...
 */
async function cleanupBackground(imageBuffer, visionResults) {
    try {
        return await (0, sharp_1.default)(imageBuffer)
            .withMetadata()
            .toBuffer();
 ...
 */
async function isolateSubject(imageBuffer, visionResults) {
    try {
        return await (0, sharp_1.default)(imageBuffer)
            .modulate({
            saturation: 1.05,
 ...
 */
async function removeBackgroundToWhite(imageBuffer, usePreview = false) {
    ...
    const originalMetadata = await (0, sharp_1.default)(imageBuffer).metadata();
    ...
    const fittedCutout = await (0, sharp_1.default)(croppedCutout)
        .resize({
        width: targetWidth,
        height: targetHeight,
        fit: "contain",
...
async function cropToContent(imageBuffer) {
    try {
        ...
        const rawData = await (0, sharp_1.default)(imageBuffer)
            .raw()
        ...
        const cropWidth = Math.min(metadata.width - cropLeft, Math.floor((right - left) + paddingX * 2));
        const cropHeight = Math.min(metadata.height - cropTop, Math.floor((bottom - top) + paddingY * 2));
 ...
 */
async function applyStudioBackground(imageBuffer, visionResults, usePreview = false) {
    try {
 ...
 */
async function uploadEnhancedImage(imageBuffer, listingId, filename) {
    try {
        ...
        const file = bucket.file(`listings/${listingId}/enhanced/${Date.now()}-${filename}`);
        ...
        throw new functions.https.HttpsError("internal", "Failed to upload enhanced image");
    }
}
/**
 * Cloud Function to save enhancement variant to Firestore
 */
exports.saveEnhancementVariant = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "User must be authenticated");
    }
    ...
    try {
        ...
        if (!quotaSnap.exists) {
            ...
            const samePeriod = quota.year === currentYear && quota.month === currentMonth;
            await quotaRef.set({
                listingId,
                year: currentYear,
                month: currentMonth,
        ...
        throw new functions.https.HttpsError("internal", "Failed to save variant");
    }
});
/**
 * Cleanup old enhancement requests
 */
exports.cleanupEnhancements = functions.pubsub
