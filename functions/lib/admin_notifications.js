...
exports.sendGlobalNotification = functions.https.onCall(async (data, context) => {
    // ---- auth guard ----
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "You must be signed in to send notifications.");
    }
    const callerUid = context.auth.uid;
    const isAdmin = await isAdminUser(callerUid);
    if (!isAdmin) {
        throw new functions.https.HttpsError("permission-denied", "Only admins can send global notifications.");
    }
    // ---- input validation ----
    const title = typeof data.title === "string" ? data.title.trim() : "";
    const body = typeof data.body === "string" ? data.body.trim() : "";
    const targetMode = data.targetMode || "topic";
    ...
    if (!["topic", "all", "self"].includes(targetMode)) {
        throw new functions.https.HttpsError("invalid-argument", "targetMode must be 'topic', 'all', or 'self'.");
    }
    ...
    if (targetMode === "self") {
        ...
        const selfTokens = Array.isArray(callerData?.fcmTokens)
