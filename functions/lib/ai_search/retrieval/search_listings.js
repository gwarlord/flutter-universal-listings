...
exports.searchListings = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        if (interpretation.filters.category) {
            query = query.where("category", "==", interpretation.filters.category.toLowerCase());
        }
        ...
        for (const doc of snapshot.docs) {
            ...
            const result = {
                ...
                explainabilityChips: generateExplainabilityChips(data, scores, userLocation ? scores.distance : undefined),
            };
            results.push(result);
        }
        ...
        logSearchResults(userId, interpretation, trimmedResults).catch((err) => {
...
function scoreResult(listing, interpretation, userLocation) {
    ...
    const searchableText = `${listing.title} ${listing.description} ${listing.category}`
        .toLowerCase();
    ...
    const relevanceScore = Math.min(100, (matchedWords.length / queryWords.length) * 150);
