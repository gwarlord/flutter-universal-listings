...
exports.searchListingsCallable = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        for (const doc of snapshot.docs) {
            ...
            if (scores.relevanceScore < MIN_RELEVANCE_SCORE)
                continue;
            const result = {
                id: doc.id,
                title: data.title || "",
...
function scoreResult(listing, interpretation, userLocation) {
    ...
    const fallbackSearchableBlob = (searchableBlob ||
        [
            titleText,
            categoryText,
            descriptionText,
    ...
    const intentScore = computeIntentScore(listing, interpretation, relevanceScore, qualityScore, freshnessScore);
    // Weighted final score
    // Relevance (45%) > Quality (20%) > Distance (15%) > Freshness (10%) > Intent-fit (10%)
    const finalScore = relevanceScore * 0.45 +
        qualityScore * 0.2 +
        distanceScore * 0.15 +
        freshnessScore * 0.1 +
        intentScore * 0.1;
...
function buildQueryTerms(inputs) {
...
function buildPhraseFromInput(input) {
...
function generateTokenVariants(token) {
...
function computeWeightedSemanticScore(terms, fields) {
    if (!terms.length)
        return 0;
...
function computePhraseScore(phrase, fields) {
    if (!phrase || phrase.length < 4)
        return 0;
    if (fields[0]?.includes(phrase))
        return 100;
    if (fields[1]?.includes(phrase))
        return 90;
    if (fields[2]?.includes(phrase))
        return 75;
    if (fields[3]?.includes(phrase))
        return 65;
    return 0;
}
function computeIntentScore(listing, interpretation, relevanceScore, qualityScore, freshnessScore) {
...
function extractServicesAndMenuText(listing) {
    const chunks = [];
    if (Array.isArray(listing.services)) {
        for (const service of listing.services) {
            if (!service || typeof service !== "object")
                continue;
            if (service.name)
                chunks.push(String(service.name));
            if (service.description)
                chunks.push(String(service.description));
        }
    }
    if (Array.isArray(listing.menuSections)) {
        for (const section of listing.menuSections) {
            if (!section || typeof section !== "object")
                continue;
            if (section.title)
                chunks.push(String(section.title));
            if (Array.isArray(section.items)) {
                for (const item of section.items) {
                    ...
                    if (item.description)
                        chunks.push(String(item.description));
                    if (Array.isArray(item.tags))
                        chunks.push(item.tags.join(" "));
                }
            }
        }
    }
...
function generateExplainabilityChips(listing, scores, distance) {
    ...
    const rating = listing.rating || listing.averageRating || (listing.reviewsSum && listing.reviewsCount ? listing.reviewsSum / listing.reviewsCount : 0);
...
exports.searchListingsRest = functions.https.onRequest(async (req, res) => {
    ...
    try {
        ...
        const interpretation = normalizeInterpretation(req.body?.interpretation);
        ...
        let candidateDocs = [];
        const canUseTargetedTokenQuery = !interpretation.filters.priceMin &&
        ...
        if (topResults.length == 0) {
            ...
            for (const doc of listingsSnapshot.docs) {
                ...
                if (listingData.suspended || listingData.hidden)
                    continue;
