...
exports.expireFeaturedListings = exports.adminApproveFeaturedRequest = exports.requestFeaturedListing = void 0;
 ...
 */
async function checkListingEligibility(listing) {
    // Check if suspended
 ...
 */
exports.requestFeaturedListing = functions
    ...
    try {
        ...
        if (listing.authorID !== uid) {
            throw new functions.https.HttpsError("permission-denied", "You can only request featured status for your own listings");
        }
        // Check eligibility
        const eligibili = await checkListingEligibility(listing);
        if (!eligibili.eligible) {
            throw new functions.https.HttpsError("failed-precondition", `Listing Not Eligible: ${eligibili.reason}`);
        }
        // Check subscription tier
        const tier = await fetchEntitlement(uid);
        if (tier < 1) {
            throw new functions.https.HttpsError("permission-denied", "You must have a Professional or Premium subscription to request featured placement");
        }
        ...
        if (!featuredRequest.empty) {
            throw new functions.https.HttpsError("already-exists", "This listing already has an active featured request");
        }
        // Create featured request (auto-approve for eligible listings)
        const now = admin.firestore.Timestamp.now();
        const expiresAt = new admin.firestore.Timestamp(now.seconds + 30 * 24 * 60 * 60, // 30 days
        now.nanoseconds);
        ...
        throw new functions.https.HttpsError("internal", "Failed to process featured request");
    }
});
/**
 * Admin function to manually approve/reject featured requests
 */
exports.adminApproveFeaturedRequest = functions
    .runWith({ enforceAppCheck: true })
    .https.onCall(async (data, context) => {
    // Verify admin
    if (!context.auth) {
    ...
    const isAdmin = context.auth.token.isAdmin === true ||
        ...
        false;
    if (!isAdmin) {
        throw new functions.https.HttpsError("permission-denied", "Only admins can approve featured requests");
    }
    const requestId = data.requestId;
    const approve = data.approve;
    if (!requestId || approve === undefined) {
        throw new functions.https.HttpsError("invalid-argument", "requestId and approve are required");
    }
    try {
        ...
        if (!requestDoc.exists) {
            throw new functions.https.HttpsError("not-found", "Featured request not found");
        }
 ...
 */
exports.expireFeaturedListings = functions.pubsub
