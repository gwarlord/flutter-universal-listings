...
export const updateBrand = functions.https.onCall(async (data, context) => {
  ...
  try {
    ...
    if (
      (patch.isVerified !== undefined || patch.freshnessExempt !== undefined) &&
      !isAdmin
    ) {
      throw new functions.https.HttpsError(
        "permission-denied",
...
export const linkListingToBrand = functions.https.onCall(
  async (data, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
        "unauthenticated",
...
export const unlinkListingFromBrand = functions.https.onCall(
  async (data, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
        "unauthenticated",
