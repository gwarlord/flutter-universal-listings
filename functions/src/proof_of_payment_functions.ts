    ...
    for (const token of tokens) {
      try {
        ...
        functions.logger.warn("POP: push token send failed", {
          ...
          error: (tokenError as any)?.message || String(tokenError),
        });
      }
    }
  } catch (error) {
...
async function sendEmail(to: string, subject: string, html: string) {
  try {
    const apiKey = sendgridKeySecret.value();
    if (!apiKey) {
 ...
 */
export const submitProofOfPayment = functions.runWith({ secrets: [sendgridKeySecret] }).https.onCall(
  async (data, context) => {
 ...
 */
export const reviewProofOfPayment = functions.runWith({ secrets: [sendgridKeySecret] }).https.onCall(
  async (data, context) => {
    // Verify authentication
    if (!context.auth) {
      throw new functions.https.HttpsError(
 ...
 */
export const onProofOfPaymentUploaded = functions.runWith({ secrets: [sendgridKeySecret] }).firestore
  ...
  .onUpdate(async (change, context) => {
    ...
    const beforeUploads: any[] = (before.proofOfPayment?.uploads) || [];
    const afterUploads: any[] = (after.proofOfPayment?.uploads) || [];

    // Only proceed if a new upload was added
    if (afterUploads.length <= beforeUploads.length) return;

    const listerUserId: string = after.listersUserId || "";
    const listerEmail: string = after.listersEmail || "";
    const customerName: string = after.customerName || "A customer";
    const listingTitle: string = after.listingTitle || "your listing";
    const orderId: string = after.id || context.params.bookingId;
    ...
    if (listerUserId) {
      await sendPushNotification(
        listerUserId,
        "New Proof of Payment",
        `${customerName} submitted proof of payment for Order #${orderId.slice(0, 8)}`,
        {
          orderId,
          listingId,
          type: "proof_of_payment_submitted",
 ...
 */
export const onProofOfPaymentReviewed = functions.runWith({ secrets: [sendgridKeySecret] }).firestore
  .document("listings/{listingId}/bookings/{bookingId}")
  .onUpdate(async (change, context) => {
    const before = change.before.data();
    const after = change.after.data();
    ...
    const prevStatus: string = (before.proofOfPayment?.overallStatus || "").toUpperCase();
    const nextStatus: string = (after.proofOfPayment?.overallStatus || "").toUpperCase();

    // Only proceed when the review decision was just set (VERIFIED or REJECTED)
    if (prevStatus === nextStatus) return;
    if (!["VERIFIED", "REJECTED"].includes(nextStatus)) return;
    // Don't fire on a fresh upload flipping from NONE → SUBMITTED (handled by onProofOfPaymentUploaded)
    if (nextStatus === "SUBMITTED") return;
    ...
    const reviewNote: string = latestUpload.reviewerNote || "";
    ...
    if (customerId) {
      await sendPushNotification(
        customerId,
        `Proof of Payment ${decisionLabel}`,
        `Your proof of payment for Order #${orderId.slice(0, 8)} has been ${decisionLower}`,
        {
          orderId,
          listingId,
          type: "proof_of_payment_reviewed",
    ...
    if (customerEmail) {
      await sendEmail(
        customerEmail,
        `Proof of Payment ${nextStatus} – Order #${orderId.slice(0, 8)}`,
        `
