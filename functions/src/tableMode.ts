 ...
 */
async function isListingOwner(listingId: string, uid: string): Promise<boolean> {
 ...
 */
async function canManageTableMode(listingId: string, uid: string): Promise<boolean> {
 ...
 */
async function generateNextTableCode(listingId: string): Promise<string> {
      ...
      await db.collection("users").doc(recipientUid).set(
        {
          ...
          pushToken:
            userData?.pushToken && invalidTokens.includes(userData.pushToken)
              ? admin.firestore.FieldValue.delete()
              : userData?.pushToken,
          fcmToken:
            userData?.fcmToken && invalidTokens.includes(userData.fcmToken)
              ? admin.firestore.FieldValue.delete()
              : userData?.fcmToken,
        },
        { merge: true }
 ...
 */
async function notifyStaffPendingSession(
  listingId: string,
  sessionId: string,
  tableName: string,
  customerName: string
  ...
  if (ownerUid) notifyUids.push(ownerUid);
...
export const setTableModeSettings = functions.https.onCall(async (data, context) => {
  try {
    ...
    if (cooldown < DEFAULT_SUMMON_COOLDOWN_SECONDS || cooldown > 600) {
      throw new functions.https.HttpsError(
        "invalid-argument",
        "summonCooldownSeconds must be between 20 and 600"
      );
    }
...
export const createTableSession = functions.https.onCall(async (data, context) => {
  ...
  const overrideExpiresAt = overrideData?.expiresAt?.toDate?.() as Date | undefined;
  const now = new Date();
  ...
  let validatedTableId: string;
  let validatedTableName: string;
 ...
 */
export const freeBlockedCustomer = functions.https.onCall(async (data, context) => {
  try {
    if (!context.auth) {
      throw new functions.https.HttpsError("unauthenticated", "User must be authenticated");
    }
    ...
    const expiresAt = admin.firestore.Timestamp.fromDate(
      new Date(Date.now() + unblockMinutes * 60 * 1000)
    );
...
export const assignWaiterToSession = functions.https.onCall(async (data, context) => {
  ...
  if (customerUid) {
    ...
    await sendPushNotification(
      customerUid,
      "Waiter Assigned",
      `${waiterNames} will assist you today`,
      {
        type: "table_session",
        sessionId,
        listingId,
...
export const summonWaiter = functions.https.onCall(async (data, context) => {
  try {
    ...
    const cooldownSeconds = Math.min(
      Number(sessionData.summonCooldownSeconds || DEFAULT_SUMMON_COOLDOWN_SECONDS),
      DEFAULT_SUMMON_COOLDOWN_SECONDS
    );
