...
export const refreshListingFreshness = functions.https.onCall(
  async (data, context) => {
    ...
    if (listing.authorID !== uid && !isAdmin) {
      throw new functions.https.HttpsError(
        "permission-denied",
        "You do not have permission to refresh this listing"
      );
    }

    const days = getFreshnessForCategory(listing.category, listing.freshness?.days);
    const now = admin.firestore.Timestamp.now();
    const hideAt = admin.firestore.Timestamp.fromMillis(
      now.toMillis() + days * DAY_MS
    );
...
export const bulkRefreshListings = functions.https.onCall(
  async (data, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
        "unauthenticated",
    ...
    if (listingIds.length > 50) {
      throw new functions.https.HttpsError(
        "invalid-argument",
        "Cannot refresh more than 50 listings at once"
      );
    }
...
async function backfillMissingFreshness(now: Date) {
...
async function backfillQuery(
  ...
  for (const doc of snap.docs) {
    ...
    const hideAt =
      resolveTimestamp(existing.hideAt) ||
      new Date(lastRefreshedAt.getTime() + days * DAY_MS);

    const freshnessUpdate: any = {
...
async function sendFreshnessWarnings(now: Date, type: "10d" | "1d") {
  ...
  for (const doc of snapshot.docs) {
    ...
    if (freshness[warnField]) continue;
    ...
    const daysRemaining = Math.max(
      0,
      Math.ceil((hideAt.getTime() - now.getTime()) / DAY_MS)
    );
...
async function hideExpiredListings(now: Date) {
  const snapshot = await db
    .collection("listings")
    .where("freshness.enabled", "==", true)
    .where("freshness.exempt", "==", false)
...
async function sendFreshnessNotification(params: {
  ...
  if (!userSnap.exists) return { emailSent, pushSent };
...
function resolveTimestamp(value: any): Date | null {
...
function resolveListingCreatedAt(listing: any, fallback: Date) {
...
function parseFreshnessDays(value: any) {
...
async function isListerExempt(userId: string) {
...
const CATEGORY_FRESHNESS: { [key: string]: number } = {
  // Short-term
  deals: 30,
  flash_sales: 14,
  events: 14,
...
function getFreshnessForCategory(
  category: string | undefined,
  customDays?: number
): number {
  if (customDays && customDays > 0) return customDays;
  if (!category) return DEFAULT_FRESHNESS_DAYS;
  ...
  .onRun(async () => {
    ...
    for (const doc of expiringListings.docs) {
      ...
      if (
        ...
        const days = getFreshnessForCategory(listing.category, listing.freshness?.days);
        const newHideAt = admin.firestore.Timestamp.fromMillis(
...
async function calculateActivityScore(listingId: string) {
  const now = new Date();
  const thirtyDaysAgo = new Date(now.getTime() - 30 * DAY_MS);
...
async function sendAutoRefreshNotification(listing: any, activityScore: any) {
...
function buildAutoRefreshEmail(listing: any, activityScore: any): string {
  return `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
